...
> Try adjusting `fig.width` and `fig.height` in your chunk options. For side-by-side plots using patchwork, `fig.width=10, fig.height=5` is a good starting point.
>
...
[Image: Vision dominates our sensory bandwidth, making it the most effective channel for communicating complex data.]
...
[Image: Preattentive processing allows us to detect certain visual features instantly, without conscious effort.]
...
[Image: Without a preattentive cue, finding the 5s requires slow, serial search.]

[Image: With color as a preattentive cue, the 5s pop out immediately.]
...
[Image: A visual catalog of preattentive attributes. Each of these features is detected automatically by the visual system.]
...
> **Tip:** **Design implication:** When you want to draw attention to a specific data point or category, encode it with a preattentive attribute -- a distinct color, a larger size, or a different shape. Do not rely on labels or legends alone for the most important comparisons.
...
[Image: The Gestalt principles describe how we perceive groups, patterns, and wholes from individual visual elements.]
...
> **Tip:** **Putting Gestalt to work:** When designing a visualization, ask yourself: "What groupings do I want the viewer to perceive?" Then use proximity, similarity, enclosure, or connection to create those groupings. If the viewer is seeing groups you did not intend, one of these principles is likely working against you.
...
[Image: Cleveland and McGill's hierarchy of perceptual accuracy, as adapted by Alberto Cairo. Encodings at the top are judged most accurately.]
...
[Image: The same data shown as a bar chart, bubble chart, and heatmap. Notice how precision decreases from left to right.]
...
[Image: Color can be described by three properties: hue, saturation, and lightness.]
...
[Image: Context effects: identical colors appear different depending on their surroundings. This is why consistent backgrounds matter in visualization.]

> **Tip:** **Rule of thumb for color:** Use color purposefully. If a color does not encode data or serve a grouping function, consider whether it is adding unnecessary visual noise. Default to a muted palette and reserve vivid colors for the elements you want to emphasize.
...
[Image: Visual working memory is limited to about 3-4 items. Exceeding this limit forces viewers to look back and forth between the legend and the chart.]
...
[Image: William Playfair's time series and bar chart from The Commercial and Political Atlas (1786) -- the earliest known examples of these chart forms.]
...
[Image: John Snow's cholera map of the Soho district, London (1854). Each small bar represents a death. The cluster around the Broad Street pump is unmistakable.]
...
[Image: Minard's map of Napoleon's march to and from Moscow (1869). The width of the band represents army size -- from 422,000 soldiers at the start to a mere 10,000 returning.]
...
> **Tip:** **Practical rule of thumb:** After creating a chart, ask yourself: "Can I remove this element without losing information?" If the answer is yes, remove it. Repeat until the answer is no for every remaining element.
...
[Image: The 'shrinking doctor' -- the figure shrinks faster than the underlying data warrants, dramatically exaggerating the decline.]

[Image: The 'shrinking dollar' -- the visual shrinkage far exceeds the actual decline in purchasing power, producing a lie factor well above 1.]
...
[Image: A slopegraph showing change across two time periods. The slope of each line immediately communicates the direction and magnitude of change.]
...
[Image: Andrew Gelman's redesigned voucher map -- a clear example of thoughtful design choices serving the data.]
...
## Challenge: Design Detective

> **Exploration Tasks:**
>
> 1. Enter your name and click **Start Game**
> 2. Each round shows a deliberately flawed chart — identify the design flaw from the options
> 3. Then rate the severity of the flaw (1-5) and compare your rating to the expert panel
