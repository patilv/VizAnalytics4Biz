---
title: "Week 1: Introduction to Data Visualization & R"
output:
  html_document:
    toc: true
    toc_float: true
...
<h1>Introduction to Data Visualization & R</h1>
<p>Why we visualize data, the power of graphical representation, and getting started with R</p>
</div>
...
```{r fun-vis, echo=FALSE, fig.cap="Visualization can be fun and engaging while remaining informative.", out.width="70%"}
knitr::include_graphics("images/01/funvis.jpg")
```
...
```{r convincing-vis, echo=FALSE, fig.cap="Effective visualizations are convincing because they communicate clearly.", out.width="70%"}
knitr::include_graphics("images/01/convincing.png")
```
...
```{r anscombe-load-packages, message=FALSE, warning=FALSE}
library(tidyverse)
```
...
```{r anscombe-reshape, message=FALSE, warning=FALSE}
...
```{r anscombe-plot, fig.width=8, fig.height=6, fig.cap="Anscombe's Quartet: four datasets with nearly identical summary statistics but very different visual patterns."}
...
```{r dino-gif, echo=FALSE, fig.cap="The Datasaurus Dozen: thirteen datasets with identical summary statistics and wildly different distributions.", out.width="80%"}
knitr::include_graphics("images/01/DinoSequential.gif")
```

We can use the `datasauRus` package to plot all thirteen datasets at once:

```{r datasaurus-load, message=FALSE, warning=FALSE}
library(datasauRus)
```

```{r datasaurus-plot, fig.width=10, fig.height=8, fig.cap="All thirteen Datasaurus Dozen datasets share nearly identical summary statistics despite their wildly different shapes."}
...
## Try It: Datasaurus Explorer {.tabset}
...
<span class="sandbox-icon">🧪</span> Datasaurus Explorer — Same Stats, Different Shapes
</div>
<iframe class="sandbox-frame" src="https://analytics.gonzaga.edu/week01-datasaurus/"
        height="560" loading="lazy" allowfullscreen></iframe>
...
```{r r-logo, echo=FALSE, fig.cap="The R Project for Statistical Computing.", out.width="20%"}
knitr::include_graphics("images/01/Rlogo.png")
```
...
```{r cran-growth, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="The growth of CRAN packages over time, generated from live CRAN data.", fig.width=8, fig.height=4}
...
```{r first-ggplot-setup, message=FALSE, warning=FALSE}
library(tidyverse)
```
...
```{r first-ggplot-basic, fig.width=7, fig.height=5}
...
```{r first-ggplot-polished, fig.width=8, fig.height=5, fig.cap="A polished scatter plot mapping engine size to highway fuel economy, colored by vehicle class."}
ggplot(mpg, aes(x = displ, y = hwy, color = class)) +
...
<div class="sandbox-section">
<div class="sandbox-header">
<span class="sandbox-icon">🎮</span> Stats Trap — Can you spot the lie hidden in the numbers?
</div>
<iframe class="sandbox-frame" src="https://analytics.gonzaga.edu/week01-game/"
...
```{r exercise4-template, eval=FALSE}
library(tidyverse)
...
<div class="page-nav">
<span></span>
<a href="week02.html">Week 2: Visual Perception & Design &rarr;</a>
</div>
