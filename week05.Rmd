---
title: "Week 5: Data Wrangling Essentials"
output:
  html_document:
    toc: true
    toc_float: true
...
<h1>Data Wrangling Essentials</h1>
<p>Tidy data, the pipe operator, and the core dplyr verbs you need for every visualization</p>
</div>
...
```{r w5-load-tidyverse, message=FALSE, warning=FALSE}
library(tidyverse)
```

```{r w5-messy-data, message=FALSE, warning=FALSE}
...
```{r w5-tidy-data, message=FALSE, warning=FALSE}
...
```{r w5-tidy-plot, fig.width=8, fig.height=4, message=FALSE, warning=FALSE}
tidy %>%
...
::: {.callout-tip}
**Rule of thumb:** If you want to map something to color, fill, or facet but the categories are stuck in column *names* rather than in a column of their own, you need `pivot_longer()`.
:::
...
```{r w5-pipe-nested, message=FALSE, warning=FALSE}
...
```{r w5-pipe-clear, message=FALSE, warning=FALSE}
...
::: {.callout-info}
**Base R pipe `|>` vs. magrittr pipe `%>%`:** R 4.1+ introduced a native pipe operator `|>`. It works similarly for most use cases. In this course we use `%>%` because it is what you will encounter in most existing tidyverse code and tutorials. Both are fine. If you see `|>` in online examples, know that it does essentially the same thing.
:::

## 3. The dplyr Verbs {.tabset}
...
```{r w5-filter-demo, message=FALSE, warning=FALSE}
...
```{r w5-select-demo, message=FALSE, warning=FALSE}
...
```{r w5-mutate-demo, message=FALSE, warning=FALSE}
...
```{r w5-summarise-ungrouped, message=FALSE, warning=FALSE}
...
```{r w5-summarise-grouped, message=FALSE, warning=FALSE}
...
```{r w5-arrange-demo, message=FALSE, warning=FALSE}
...
You have just learned the six core dplyr verbs. Now build a data transformation pipeline interactively. The sandbox below lets you toggle each verb on and off to see how the data changes at each step — no need to write code.

<div class="sandbox-section">
<div class="sandbox-header">
<span class="sandbox-icon">🧪</span> dplyr Pipeline Builder — Transform Data Step by Step
</div>
<iframe class="sandbox-frame" src="https://analytics.gonzaga.edu/week05-pipeline/"
...
::: {.callout-ai}
**AI & This Concept**
When asking AI to wrangle data, describe your pipeline step by step: "Filter to rows where year > 2000, then group by country, then compute mean GDP per capita." AI tools produce much cleaner dplyr code when you break the transformation into explicit steps rather than asking for the end result all at once.
:::
...
```{r w5-pivot-step1, message=FALSE, warning=FALSE}
...
```{r w5-pivot-step2, message=FALSE, warning=FALSE}
...
```{r w5-pivot-step3, fig.width=8, fig.height=5, message=FALSE, warning=FALSE}
...
::: {.callout-warning}
**Common pitfall:** Forgetting to pivot before plotting. If you try to map both `cty` and `hwy` to the y-axis in a single ggplot call, you will run into trouble. The tidy approach is always: **pivot first, then map** the new category column to `fill` or `color`.
:::
...
<div class="sandbox-section">
<div class="sandbox-header">
<span class="sandbox-icon">🧪</span> Tidy Data Transformer — See pivot_longer() in Action
</div>
<iframe class="sandbox-frame" src="https://analytics.gonzaga.edu/week05-tidydata/"
...
::: {.callout-ai}
**AI & This Concept**
When asking AI to reshape data, describe what the wide and long versions should look like: "I have columns Q1, Q2, Q3, Q4 and I want a single 'quarter' column with a 'revenue' column for the values." Giving AI a concrete before-and-after description produces much more reliable `pivot_longer()` code than vague requests like "make my data tidy."
:::
...
```{r w5-fct-unsorted, fig.width=8, fig.height=4.5, message=FALSE, warning=FALSE}
...
```{r w5-fct-sorted, fig.width=8, fig.height=4.5, message=FALSE, warning=FALSE}
...
::: {.callout-tip}
**`fct_reorder()` is your best friend for bar charts.** The pattern is always the same: `fct_reorder(category_column, numeric_column)`. This reorders the categories so that they appear in order of the numeric value. It works with horizontal bars (`y = fct_reorder(...)`) and vertical bars (`x = fct_reorder(...)`).
:::
...
```{r w5-stringr-brief, message=FALSE, warning=FALSE}
...
```{r w5-pipeline-step1, message=FALSE, warning=FALSE}
...
```{r w5-pipeline-step2, message=FALSE, warning=FALSE}
...
```{r w5-pipeline-step3, message=FALSE, warning=FALSE}
...
```{r w5-pipeline-step4, message=FALSE, warning=FALSE}
...
```{r w5-pipeline-step5, fig.width=10, fig.height=5, message=FALSE, warning=FALSE}
...
::: {.callout-info}
**Why separate the wrangling from the plotting?** Keeping them in distinct steps (even if you could combine them) makes your code easier to debug. If the plot looks wrong, you can inspect the intermediate data frame (`economics_clean`) to see whether the problem is in the wrangling or the visualization. Debug one thing at a time.
:::
...
<div class="sandbox-section">
<div class="sandbox-header">
<span class="sandbox-icon">🎮</span> Pipe Dream — Get the verbs in order or the pipeline breaks
</div>
<iframe class="sandbox-frame" src="https://analytics.gonzaga.edu/week05-game/"
...
```{r w5-exercise1-starter, eval=FALSE}
storms %>%
  filter(___ >= 4) %>%
  group_by(___) %>%
...
```{r w5-exercise2-starter, eval=FALSE}
...
```{r w5-exercise3-starter, eval=FALSE}
mpg %>%
  filter(year == ___) %>%
  group_by(___) %>%
...
<div class="page-nav">
<a href="week04.html">&larr; Week 4</a>
<a href="week06.html">Week 6 &rarr;</a>
</div>
